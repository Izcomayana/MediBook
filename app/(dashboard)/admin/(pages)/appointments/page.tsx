...
function AppointmentCard({
  appt, onUpdate, updating,
}: { appt: Appointment; onUpdate: (id: string, s: "confirmed" | "cancelled") => void; updating: string | null }) {
  return (
        ...
        <div className="flex min-h-screen w-full bg-[#0a1f15]">
          ...
          <main className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-1 px-6 py-6 space-y-5 max-w-6xl mx-auto w-full">
              ...
              {/* Result count */}
              {!loading && (
                <p className="text-xs text-[#3d6b54]">{filtered.length} appointment{filtered.length !== 1 ? "s" : ""}</p>
              )}
                      ...
                      <tbody className="divide-y divide-[#1d3a28]">
                        {filtered.map((appt, i) => (
                          <tr
                            ...
                            style={{ animation: `fadeSlideUp 0.4s ease ${i * 40}ms both` }}
                          >
